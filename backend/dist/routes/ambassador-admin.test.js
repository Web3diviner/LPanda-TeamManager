...
const mockConnect = db_1.default.connect;
const JWT_SECRET = process.env.JWT_SECRET ?? 'dev-secret-change-in-production';
function makeToken(role) {
    return jsonwebtoken_1.default.sign({ sub: 'user-uuid-1', role, name: 'Test User' }, JWT_SECRET, { expiresIn: '1h' });
}
...
describe('requireAmbassadorAdmin middleware', () => {
    it('returns 401 with no token', async () => {
        const res = await (0, supertest_1.default)(index_1.default).get('/ambassador-admin/ambassadors');
...
describe('GET /ambassador-admin/ambassadors — scope isolation', () => {
    it('returns only ambassador-role users', async () => {
        ...
        res.body.forEach((u) => expect(u.role).toBe('ambassador'));
    });
});
...
describe('POST /ambassador-admin/ambassadors/:id/adjust-points — silent deduction', () => {
    it('adjusts points without inserting a notification', async () => {
        ...
        const allCalls = mockClient.query.mock.calls.map((c) => (typeof c[0] === 'string' ? c[0] : ''));
        const notificationInserts = allCalls.filter((q) => q.toLowerCase().includes('notifications'));
