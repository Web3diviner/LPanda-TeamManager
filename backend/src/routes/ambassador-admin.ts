...
import pool from '../db';
import { authMiddleware, requireAmbassadorAdmin } from '../middleware/auth';
...
router.get('/ambassadors', authMiddleware, requireAmbassadorAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
      `SELECT id, name, email, role, role_title, points, avatar_url, created_at
       FROM users
...
router.get('/ambassadors/:id', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      `SELECT id, name, email, role, role_title, points, avatar_url, created_at
...
router.delete('/ambassadors/:id', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
...
router.post('/ambassadors/:id/adjust-points', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
...
router.get('/pending-tasks', authMiddleware, requireAmbassadorAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
...
router.get('/tasks', authMiddleware, requireAmbassadorAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
      `SELECT t.id, t.description, t.status, t.deadline, t.submitted_at, t.completed_at,
              t.screenshot_url, t.task_link,
...
router.post('/tasks/:id/assign', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  try {
    const assigneeCheck = await pool.query(`SELECT id, name FROM users WHERE id = $1 AND role = 'ambassador'`, [assigned_to]);
    if (assigneeCheck.rows.length === 0) {
      res.status(400).json({ error: 'assigned_to must be an ambassador' });
      return;
    }
    const taskResult = await pool.query(
      `UPDATE tasks SET status = 'assigned', assigned_to = $1, deadline = $2
    ...
    await pool.query(
      ...
      [randomUUID(), assigned_to, `📋 A task has been assigned to you by the ambassador admin.`],
    );
...
router.post('/tasks/:id/confirm', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const taskResult = await client.query(
      `SELECT t.*, su.role AS submitter_role, su.name AS submitter_name
       FROM tasks t
       INNER JOIN users su ON su.id = t.submitted_by
    ...
    await PointsService.award(task.submitted_by, id, client);
    await client.query(
      `INSERT INTO notifications (id, user_id, message) VALUES ($1,$2,$3)`,
      [randomUUID(), task.submitted_by, `⭐ Your task has been confirmed! Points awarded.`],
    );
    await client.query('COMMIT');
...
router.delete('/tasks/:id', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  try {
    const taskResult = await pool.query(
      `SELECT t.id, t.status, t.submitted_by, su.role
       FROM tasks t
       INNER JOIN users su ON su.id = t.submitted_by
       WHERE t.id = $1 AND su.role = 'ambassador'`,
      [id],
    );
    if (taskResult.rows.length === 0) {
...
router.get('/delegated', authMiddleware, requireAmbassadorAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
...
router.post('/delegated', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    for (const assigneeId of assigned_to) {
      ...
      const result = await client.query(
        `INSERT INTO delegated_tasks (id, title, description, assigned_to, created_by, deadline)
         VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
        [id, title, description || null, assigneeId, createdBy, deadline],
      );
      await client.query(
        `INSERT INTO notifications (id, user_id, message) VALUES ($1,$2,$3)`,
        [randomUUID(), assigneeId, `🎯 New task delegated to you: "${title}"`],
      );
...
router.delete('/delegated/:id', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  try {
    const check = await pool.query(
      `SELECT d.id FROM delegated_tasks d
       INNER JOIN users au ON au.id = d.assigned_to AND au.role = 'ambassador'
       WHERE d.id = $1`,
      [id],
    );
    ...
    await pool.query('DELETE FROM delegated_tasks WHERE id = $1', [id]);
    res.status(204).send();
  } catch (err) {
...
router.get('/leaderboard', authMiddleware, requireAmbassadorAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
...
router.get('/schedule', authMiddleware, requireAmbassadorAdmin, async (_req: Request, res: Response): Promise<void> => {
  try {
    const result = await pool.query(
      `SELECT s.id, s.title, s.description, s.day_of_week, s.time_slot, s.created_at,
...
router.post('/schedule', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const id = randomUUID();
    const result = await pool.query(
      `INSERT INTO ambassador_schedule_items (id, title, description, day_of_week, time_slot, created_by, created_at)
       VALUES ($1,$2,$3,$4,$5,$6,now()) RETURNING *`,
      [id, title, description || null, day_of_week, time_slot, req.user!.sub],
    );
...
router.delete('/schedule/:id', authMiddleware, requireAmbassadorAdmin, async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      `DELETE FROM ambassador_schedule_items WHERE id = $1 RETURNING id`,
      [id],
    );
    if (result.rows.length === 0) {
